...
async def test_manual_relationship_and_graph(client: AsyncClient):
    ...
    res_a = await client.post(
        "/api/v1/assets",
        headers=headers,
        json={"type": "domain", "value": "parent-domain.com"},
    )
    res_b = await client.post(
        "/api/v1/assets",
        headers=headers,
        json={"type": "subdomain", "value": "sub.parent-domain.com"},
    )
...
async def test_relationship_tenant_isolation(client: AsyncClient):
    ...
    res_t1 = await client.post(
        "/api/v1/assets",
        headers={"X-API-Key": TENANT_1_KEY},
        json={"type": "domain", "value": "tenant1-domain.com"},
    )
    # 2. Create asset in Tenant 2
    res_t2 = await client.post(
        "/api/v1/assets",
        headers={"X-API-Key": TENANT_2_KEY},
        json={"type": "domain", "value": "tenant2-domain.com"},
    )
    ...
    res_link = await client.post(
        "/api/v1/relationships", headers={"X-API-Key": TENANT_1_KEY}, json=rel_data
    )
...
async def test_import_integration_with_provided_json(client: AsyncClient, db_session):
    ...
    data_json_path = Path("d:/NU/Assessment/Data.json")
    with open(data_json_path, "r", encoding="utf-8") as f:
        import_data = json.load(f)

    # 2. POST to import endpoint
    response = await client.post(
        "/api/v1/assets/import", headers=headers, json=import_data
    )
    ...
    res_graph = await client.get(
        f"/api/v1/assets/{subdomain_asset.id}/graph", headers=headers
    )
