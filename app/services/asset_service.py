...
async def get_assets(
    ...
    sort_col = (
        ...
        else Asset.first_seen if sort_by == "first_seen" else Asset.value
    )
    ...
    if pagination_type == "keyset":
        ...
        if len(assets) == limit:
            ...
            cursor_val = (
                getattr(last_item, sort_by)
                if hasattr(last_item, sort_by)
                else last_item.last_seen
            )
...
async def create_asset(
    db: AsyncSession, tenant_id: UUID, asset_in: AssetCreate
) -> Asset:
    ...
    if dup.scalar_one_or_none():
        raise ValueError(
            f"Asset of type '{asset_in.type.value}' with value '{asset_in.value}' already exists."
        )
...
async def update_asset(
    db: AsyncSession, tenant_id: UUID, asset_id: UUID, asset_in: AssetUpdate
) -> Optional[Asset]:
...
async def bulk_import_assets(
    db: AsyncSession, tenant_id: UUID, raw_items: list[dict]
) -> BulkImportResponse:
    """
    ...
    for idx, raw in enumerate(raw_items):
        try:
            ...
            errors.append(
                ImportErrorDetail(index=idx, value=raw.get("value"), errors=error_msgs)
            )
    ...
    tags_union_subquery = sa.func.array(
        ...
        .select_from(
            sa.func.unnest(
                sa.func.array_cat(
                    sa.func.coalesce(
                        Asset.tags, sa.cast(sa.func.array([]), ARRAY(sa.String))
                    ),
                    sa.func.coalesce(
                        stmt.excluded.tags, sa.cast(sa.func.array([]), ARRAY(sa.String))
                    ),
                )
            ).alias("val")
        )
    ...
    for item in valid_items:
        ...
        for rel_type, rel_target in extra_fields.items():
            ...
            targets = (
                ...
                else rel_target if isinstance(rel_target, list) else []
            )
    ...
    if relationships_to_insert:
        ...
        rel_update_stmt = rel_stmt.on_conflict_do_nothing(
            constraint=(
                "uq_from_to_type" if hasattr(Relationship, "__table_args__") else None
            ),
        ...
        try:
            ...
            warnings.append(
                "Some relationships were skipped due to referential integrity (target asset does not exist)."
            )
