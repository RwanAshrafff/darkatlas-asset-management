...
async def create_relationship(
    ...
    if not from_asset:
        raise ValueError(f"Source asset {rel_in.from_asset_id} not found in this tenant.")
    if not to_asset:
...
async def get_asset_graph(
    ...
    outgoing_stmt = sa.select(Relationship, Asset).join(
        Asset, Relationship.to_asset_id == Asset.id
    ).where(
    ...
    incoming_stmt = sa.select(Relationship, Asset).join(
        Asset, Relationship.from_asset_id == Asset.id
    ).where(
