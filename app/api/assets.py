...
from app.schemas import AssetCreate, AssetUpdate, AssetResponse, BulkImportResponse
...
@router.get(
    "",
    response_model=PaginatedAssetsResponse,
    summary="List and search assets with filtering and pagination",
)
async def list_assets(
    ...
    search: Optional[str] = Query(
        None, description="Search value field (partial case-insensitive match)"
    ),
    sort_by: str = Query(
        "last_seen", description="Field to sort by (last_seen, first_seen, value)"
    ),
    sort_order: str = Query("desc", description="Sort direction (asc, desc)"),
    pagination_type: str = Query(
        "offset", description="Pagination type (offset or keyset)"
    ),
...
@router.get(
    "/{id}", response_model=AssetResponse, summary="Retrieve a single asset details"
)
async def get_asset(
    id: UUID,
    tenant_id: UUID = Depends(get_current_tenant_id),
...
@router.put(
    "/{id}", response_model=AssetResponse, summary="Update an existing asset details"
)
...
@router.delete(
    "/{id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete an asset"
)
...
@router.post(
    ...
    summary="Bulk import a list of assets and establish relations",
)
...
@router.post(
    ...
    summary="Transition inactive assets to stale status",
)
